...
object Monoid {
  ...
  def foldLeft[A, B](as: List[A])(z: B)(f: (B, A) => B): B =
    foldMap(as, this dual endoMonoid[B])(a => f(_, a))(z)
  ...
  def count(s: String): Int = {
    ...
    def countAsWord(s: String) = if (s.isEmpty) 0 else 1
    Par.run(es) {
      parFoldMap(s, wcMonoid) { ss =>
        if (ss.isWhitespace) Part("", 0, "") else Stub(ss toString)
      }
    } match {
      case Part(l, w, r) => es.shutdown(); countAsWord(l) + w + countAsWord(r)
      case Stub(w) => es.shutdown(); countAsWord(w)
    }
  }
